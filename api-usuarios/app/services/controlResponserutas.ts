...
export async function controlResponserutas(params) {
  ...
  try {
    ...
    let query = `UPDATE jsonplaceholder.transacciones SET catch_respuesta = 'catch-error: ${error.code + ' - ' +error.sqlMessage.replace(/['"]+/g, '') ? error.sqlMessage.replace(/['"]+/g, '') : error.sqlMessage}' WHERE id = (SELECT max(id) id FROM transacciones)`;
    await database.rawQuery(query);
    throw error
  }
  return 'Ok'
